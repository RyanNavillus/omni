...
def generate_world_og(world, player):
    ...
    for x in range(world.area[0]):
        for y in range(world.area[1]):
            _set_material_og(world, (x, y), player, tunnels, simplex)
    for x in range(world.area[0]):
        for y in range(world.area[1]):
            _set_object(world, (x, y), player, tunnels)


def _set_material_og(world, pos, player, tunnels, simplex):
    x, y = pos
    ...
    water = simplex(x, y, 3, {15: 1, 5: 0.15}, True) + 0.1
    water -= 2 * start
    ...
    if start > 0.5:
        ...
        if (simplex(x, y, 6, 7) > 0.15 and mountain > 0.3):  # cave
            ...
            world[x, y] = 'path'
            tunnels[x, y] = True
        elif simplex(x, y, 1, 8) > 0 and uniform() > 0.75:
            world[x, y] = 'coal'
        elif simplex(x, y, 2, 6) > 0.4 and uniform() > 0.0:
            world[x, y] = 'iron'
        elif mountain > 0.18 and uniform() > 0.98:
            world[x, y] = 'diamond'
        else:
            world[x, y] = 'stone'
