...
class Env(BaseClass):
    ...
    def __init__(
        ...
        self._item_rows = int(np.ceil(len(constants.items) / view[0]))
        self._local_view = engine.LocalView(
            self._world, self._textures, [view[0], view[1] - self._item_rows])
        self._item_view = engine.ItemView(
            self._textures, [view[0], self._item_rows])
    ...
    def step(self, action):
        self._step += 1
        # self._update_time()
        self._player.action = constants.actions[int(action)]
    ...
    def render(self, size=None, semantic=False, add_desc=False):
        if semantic:
            canvas = self._local_view.sem_call(self._player)
            return canvas
        else:
