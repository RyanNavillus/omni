...
DUMMY_BITS = env_tr_uni.DUMMY_BITS
DUMMY_TASKS = env_tr_uni.DUMMY_TASKS


class Env(env_tr_uni.Env):
    ...
    def __init__(
        ...
        self.task_probs = np.ones(len(self.target_achievements) + DUMMY_TASKS) / \
            (len(self.target_achievements) + DUMMY_TASKS)
        # MoI attributes
        self.moi_int_tasks = set()
        self.moi_bor_tasks = set()
        self.moi_predicts = get_task_predictions(setting='r')

    def _specify_curri_task(self):
        # print(self.target_achievements)
        # task_idxs = [75, 60, 10]
        self.task_idx = np.random.choice(np.arange(len(self.target_achievements) +
                                         DUMMY_TASKS), size=1, p=self.task_probs)[0]
        self.task_steps = 0
        # self.task_idx = 10
